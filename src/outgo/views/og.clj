...
(defn get-value [username today-outgo field]
    ...
    (for [field fields]
      (html (label {:class "column label-part"} field field)
            (for [username usernames]
              (text-field {:class "column one.column"} (str username "[" (name field) "]") (get-value username today-outgo field)))
            [:div {:class "clear"}]
            (label {:class "column label-part"} "Note" "Note")
            (for [username usernames]
              (text-area {:class "column one.column"} (str username "[" (name field) "_note" "]") (get-value username today-outgo (keyword (subs (str field "_note") 1)))))
            [:div {:class "clear"}]))))
